...
export const LayoutSelection = ({ setActiveStep }: LayoutSelectionProps) => {
  ...
  const handleSelect = ({ currentSelection, activeOption }: { currentSelection: LAYOUT_DETAIL_TYPE['selection']; activeOption: LAYOUT_DETAIL_TYPE['selection']['options'][0]['options'][0] }) => {
  ...
  return (
    <Flex w='100%' h='100%' minH='fit-content' flexDir='column' px='md' py='32px'>
      {activeSelection > 0 && (
        <Flex alignItems='center' gap='8px' marginBottom='19px' onClick={handleGoBack} pos='absolute' top='32px' left='32px' display={{ base: 'none', xl: 'flex' }} cursor='pointer'>
      ...
      <Flex h='100%' flexDir='column' justifyContent={{ base: 'space-between', xl: 'center' }}>
        <Flex flexDir='column' gap='60px' alignItems='center' mb={{ base: '20px', xl: '60px' }}>
        ...
        <Flex gap={{ base: '8px', xl: 'sm' }} alignItems='center' justifyContent='center' flexDir={{ base: 'column', xl: 'row' }} flex={{ base: 1, xl: 'unset' }}>
          {LAYOUT_DETAILS?.[activeSelection]?.selection?.options?.map((option) => {
            const currentSelection = LAYOUT_DETAILS?.[activeSelection]?.selection
            const matchSelection = option?.match?.selection
            const matchSelectionOption = option?.match?.option

            const isSelectionMatched = !matchSelection || matchSelection in selection?.selectionData
            const isSelectionOptionMatched =
              !matchSelectionOption ||
              matchSelectionOption ===
                // @ts-ignore
                selection?.selectionData?.[matchSelection]
                  ...
                  >
                    <Flex w='100%' flexDir='column' pos='relative'>
                      ...
                      <Flex h='100%' alignItems='center'>
                        <Image w='auto' h={{ base: '100%', xl: 'auto' }} src={activeOption?.img} alt={activeOption?.name} mx={{ base: 'unset', xl: 'auto' }} />
                      </Flex>
                    </Flex>
                  </Flex>
                )
